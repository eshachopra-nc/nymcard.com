...
**Contrast archetypes (added 2026-06-04).** The kit above gave structural variety but was still LOW-CONTRAST — every archetype was a light, body-scale, contained beat, so a page composed from them read as one uniform light scroll (owner verdict on `/solutions/embedded-finance`: "it's the same treatment in every section, just a bunch of cards… we've run out of variety and contrast"). These four exist to supply the missing contrast devices — dark full-bleed, oversized type, big figures, horizontal — so a page can ALTERNATE light ↔ dark, body ↔ display scale, contained ↔ full-bleed, vertical ↔ horizontal:

| Archetype | Structure | Use for |
| --- | --- | --- |
| **`StatementBand`** | The **EDITORIAL-BAND family head**: ONE oversized editorial statement (display scale) + a CLEAN supporting treatment, never a card grid, never a widget. **Two surfaces** via `surface`: `dark` (default) is the **FULL-BLEED always-dark** contrast anchor on a deep cool `GlassAtmosphere` field; `light` is a light editorial band on the section surface (theme-aware, NO atmosphere wash — light-first restraint, the caller wraps it in its own `<Section>`). **Three supporting shapes** via `support`: `items-row` (a hairline row of label + one-liner; items with `href` become links with a hover-nudge arrow, items without are static — no dead href), `numbered` (a clean numbered editorial row 01→…, no spine/nodes), `two-col` (statement + body hold the left column, the items run as a hairline list on the right). Optional `body` (a supporting paragraph) and `lede` (mono uppercase). Backward-compatible: `tone` ("indigo"/"cyan"/"violet"/"azure") still selects the dark field's atmosphere. | Composing a whole content page as ONE coherent family of bold editorial bands — alternate `surface` (light ↔ dark) and `support` shape for variety WITHOUT a card grid. The `dark` surface is the page's CONTRAST ANCHOR / dark beat; `light` surfaces carry the body sections in the same confident register. |
| **`BigFigureRow`** | Proof figures at **OVERSIZED display scale** (text-6xl+), hairline-separated, airy. `tone="light"` on the section surface; `tone="dark"` a SELF-CONTAINED full-bleed deep-cool band (own field + padding + crosshairs + optional `heading`). | The proof-of-scale moment where the numbers ARE the design — the scale-up of `StatBand`. The dark tone is a considered dark proof beat. |
| **`OversizedEditorialSplit`** | A **DISPLAY-scale headline** on the wider column ↔ a tight hairline list on the narrower column. The headline is the dominant object. No cards, no glass. | The "oversized type" beat — a problem/statement opener where scale-contrast against the body sections does the work. |
| **`HorizontalRow`** | A **horizontal** hairline-divided RAIL of tall typographic panels (index numeral + icon + name + one-liner) that reads SIDEWAYS; swipe-snap on mobile. Mixed linked/static: items with `href` are Links (hover-lift + arrow), items without are static panels (no dead href). | A set of peers (industries, use-cases, segments) WITHOUT a card grid — the horizontal axis is the contrast. |

**Scrollytelling archetype (added 2026-06-04 — the flagship "how it works" beat).** The kit had no PINNED-VISUAL treatment, so a multi-step narrative (the nCore foundation story: One Customer → Data Layer → Intelligence Layer) resolved to N stacked alternating rows — readable, but it never read as ONE continuous idea, and it had no signature moment. `StickyScroll` is that moment:

| Archetype | Structure | Use for |
| --- | --- | --- |
| **`StickyScroll`** | A visual column that STAYS PINNED (CSS `position: sticky` — **no scroll hijacking**, the page scrolls natively) on one side while the copy column scrolls through N steps. As each copy panel crosses the viewport centre an `IntersectionObserver` sets it active; the pinned visual **cross-fades** (AnimatePresence, `dur.slow`/`ease.out`) to that step's `visual`. A hairline spine threads the steps with a node marker that fills on the active step; inactive steps dim back (still readable). Optional `headline`/`lede` opener (omit to let the steps lead — no invented umbrella copy). | The premium **"how it works"** / multi-layer narrative beat — the page's SIGNATURE moment, where one surface carries the whole story. `steps: [{ title, body, visual: ReactNode }]`; the `visual` is any node (a labelled `UIPlaceholder`, or later a real product-UI). |

**`StickyScroll` reduced-motion / no-JS / mobile behaviour (the contract):** `prefers-reduced-motion`, SSR/no-JS, AND narrow viewports (`< lg`) all render a **PLAIN STACKED LIST** — each step shows its title, body and its OWN `visual` inline, in document order, with NO pin, NO cross-fade, NO observer-driven dimming and NO scroll-jacking. The sticky + observer layer is a pure enhancement layered on top of that baseline (mounted client-side, `lg`-only, motion-safe), so the content is complete and ordered without it. Client component (it owns the observer + the cross-fade); the per-step `visual` is server-rendered and passed in. **First applied:** `/platform/ncore` §4–6 — the One Customer / Data Layer / Intelligence Layer copy verbatim across three steps over one pinned `UIPlaceholder` surface (the page's signature moment).

**Contrast rules (a page composed from this kit MUST satisfy):**
- **No two sections share a treatment.** A card grid may appear AT MOST ONCE on a page, never as the default.
- **Alternate, don't accumulate.** Contrast comes from alternation (light ↔ dark, dense ↔ airy, body ↔ display type, contained ↔ full-bleed), not more cards. A page needs at least **two non-adjacent dark beats**, at least one **oversized-type** moment, and at least one **full-bleed** moment.
- **Dark beats are `StatementBand`, `BigFigureRow tone="dark"`, or `BridgeBand`** — all self-contained, always-dark, full-bleed-able. Never place two adjacent.

**Shared motion primitive:** `StaggerList` (`archetypes/Reveal.tsx`) — every archetype's list reveals through it, so the whole kit shares one restrained beat (mirrors `SectionReveal`, resolved per-item). `HorizontalRow` and `StickyScroll` are client components (`HorizontalRow`: scroll-reveal + scroll-snap rail; `StickyScroll`: the IntersectionObserver + cross-fade); the others are server components (the dark fields compose only static layers + the client `GlassAtmosphere`).
...
- **Reference pages:** `app/(site)/solutions/exchange-houses` is the original (structural-variety) reference. **`app/(site)/solutions/embedded-finance` is the EDITORIAL-BAND template (2026-06-04, second pass)** — the owner reviewed the contrast-archetype rebuild and decided the ONE section that worked was §4 (the full-bleed dark `StatementBand`); the others (oversized split, glass stepper, process rail, horizontal scroller) read as fussy "designed widgets". Verdict: rebuild §2/3/5/6 in §4's bold, confident editorial-band register — strong headline + a clean supporting row/list — and drop the widgets. So §2/3/5/6 are now all `StatementBand` (the extended family head), varied by `surface` + `support` shape: §2 `surface="dark"` items-row (the opening dark beat, + body), §3 `surface="light"` two-col, §4 `surface="dark"` items-row (the band the owner kept — UNTOUCHED), §5 `surface="light"` numbered, §6 `surface="light"` linked items-row, §7 `BigFigureRow tone="dark"` (UNTOUCHED), §8 `FinalCTA` (UNTOUCHED). Light/dark rhythm `light · DARK · light · DARK · soft · light · DARK · light` — three non-adjacent dark beats (§2, §4, §7); the family stays coherent (all bold bands) yet varied (tone alternation + supporting-shape + density). NO card grid anywhere. The prior contrast-archetype build is archived in `components/sections/solutions/versions/`.
...
- **v2.7 (June 2026 — editorial-band family, owner direction):** **`StatementBand` (§8.32) extended into the EDITORIAL-BAND family head.** The owner reviewed the v2.6 contrast-archetype rebuild of `/solutions/embedded-finance` and decided the ONE section that worked was §4 (the full-bleed dark `StatementBand`); the other content sections (the oversized split, the glass `ConnectedStepper`, the `ProcessRail` spine, the `HorizontalRow` scroller) read as fussy "designed widgets". Verdict: rebuild §2/3/5/6 in §4's bold, confident editorial-band register — a strong headline + a CLEAN supporting row/list — and drop the widgets. `StatementBand` gained a **`surface="dark"|"light"`** prop (the always-dark full-bleed contrast anchor, OR a light editorial band on the section surface — theme-aware, no atmosphere wash), an optional **`body`** paragraph, and three **`support`** shapes (`items-row` with optional per-item links, `numbered`, `two-col`) so a whole page can compose from this single archetype and still read as one coherent family, varied by `surface` + `support` + density. (`tone` kept as the dark field's atmosphere selector for backward compatibility — §4 and the fintechs `BuildOnceExpand` are untouched.) **`/solutions/embedded-finance` rebuilt as the EDITORIAL-BAND TEMPLATE**: §2 `surface="dark"` items-row (+ body, the opening dark beat), §3 `surface="light"` two-col, §4 unchanged, §5 `surface="light"` numbered, §6 `surface="light"` linked items-row; §7 (`BigFigureRow tone="dark"`) and §8 (`FinalCTA`) untouched. Rhythm `light · DARK · light · DARK · soft · light · DARK · light` — three non-adjacent dark beats, no card grid anywhere. The `/visual-system` "StatementBand" demo now shows the full family (dark anchor + light variant × the three supporting shapes, light + dark). Prior section versions archived in `components/sections/solutions/versions/`; the v2.6 contrast archetypes (`OversizedEditorialSplit`, `HorizontalRow`, `ProcessRail`, `ConnectedStepper`) remain in the kit for other pages.
- **v2.6 (June 2026 — contrast archetypes, owner direction):** **§8.32 extended with four CONTRAST archetypes.** The owner's verdict on `/solutions/embedded-finance` — "it's the same treatment in every section, just a bunch of cards… we've run out of variety and contrast" — exposed that the §8.32 kit, while structurally varied, was uniformly light, body-scale and contained, so a page composed from it still read as one flat scroll. Added the missing contrast devices, each documented in §8.32 with a live light+dark `/visual-system` demo: **`StatementBand`** (full-bleed DARK band, one oversized display statement + optional hairline item row — the contrast anchor), **`BigFigureRow`** (proof figures at display scale, airy; `tone="light"` surface beat or `tone="dark"` self-contained full-bleed proof band), **`OversizedEditorialSplit`** (display-scale headline ↔ tight hairline list — the oversized-type beat), **`HorizontalRow`** (a horizontal scroll/snap rail of typographic panels with mixed linked/static cells — the explicit card-grid alternative for a set of peers). Codified the **contrast rules**: no two sections share a treatment; ≤1 card grid per page, never the default; ≥2 non-adjacent dark beats + ≥1 oversized-type + ≥1 full-bleed moment; contrast from alternation, not more cards. **`/solutions/embedded-finance` rebuilt on these as the CONTRAST TEMPLATE** for the other 3 use-case pages — every section a distinct archetype, ZERO card grids, dark beats at §4 (`StatementBand`) and §7 (`BigFigureRow tone="dark"`), oversized type at §2 + §7, full-bleed at §4 + §7. The prior versions are archived in `components/sections/solutions/versions/`. Hero (`PageHero`, textOnly) untouched. No bespoke product illustrations added (owner deferred — contrast comes from layout/type/colour/dark fields).
