...
//
// Full country list for the Contact form's phone field, sorted by country name.
// `iso` is the unique key (dial codes are not unique — e.g. +1 covers the US,
// Canada and much of the Caribbean). Value submitted is `${iso} ${dial}` so the
// originating country is preserved even when the code collides.

export type DialCode = { name: string; iso: string; dial: string };
