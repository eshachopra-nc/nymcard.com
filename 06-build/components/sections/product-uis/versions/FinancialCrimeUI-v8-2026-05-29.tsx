...
//
// Motion:
//   · Entrance — the scan sweep crosses the four lanes once on scroll-into-view.
//   · Hover — the parent bento cell is `group`-classed; on hover the row of
//     signal-lane checks brightens in sequence (a quick staggered re-affirm)
//     and the "Cleared" decision chip warms. Reduced-motion safe (no sweep, no
//     hover transition beyond colour).
      ...
      <div className="relative flex h-full w-full flex-col justify-center gap-3.5 p-6 sm:p-8">
        ...
        <div className="relative overflow-hidden rounded-md ring-1 ring-surface-border-subtle dark:ring-white/10">
          <div className="divide-y divide-surface-border-subtle dark:divide-white/10">
            {LANES.map((lane, i) => (
              <div key={lane} className="flex items-center justify-between gap-3 bg-surface-white/60 px-3 py-1.5 transition-colors duration-300 group-hover:bg-accent-cyan/[0.04] dark:bg-white/[0.02] dark:group-hover:bg-accent-cyan/[0.06]">
                ...
                <span className="inline-flex items-center gap-1.5">
                  ...
                  {/* Check brightens in sequence on hover (staggered ring). */}
                  <span
                    className="grid size-3.5 place-items-center rounded-full bg-accent-cyan/20 ring-1 ring-accent-cyan/50 transition-all duration-300 group-hover:bg-accent-cyan/30 group-hover:ring-accent-cyan/80"
        ...
        <div className="flex items-center justify-between">
          <span className="inline-flex items-center gap-1.5 rounded-md bg-accent-cyan/[0.12] px-2.5 py-1 ring-1 ring-inset ring-accent-cyan/40 transition-colors duration-300 group-hover:bg-accent-cyan/[0.2] group-hover:ring-accent-cyan/60 dark:bg-accent-cyan/[0.16] dark:ring-accent-cyan/45 dark:group-hover:bg-accent-cyan/[0.24]">
