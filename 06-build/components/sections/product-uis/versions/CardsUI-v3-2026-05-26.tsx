...
// ── CardsUI (v3) ───────────────────────────────────────────────────────────
//
// Refinement: now inherits ProductUISurface (cyan top hairline + corner
// crosshairs + LIVE marker) — same chrome as the rest of the product
// family. The card+ribbon composition stays, with two changes:
//   • Bed ribbon opacity dropped 0.30 → 0.22 so it stops dominating its
//     siblings in the grid.
//   • Resting cyan chip-pulse on the card chip on the 8s beat.
//   • Ribbon kinetic parameters preserved — don't break what works.
...
export function CardsUI() {
  ...
  return (
    <ProductUISurface label="Cards" live contentClassName="px-0 pb-0 pt-0">
      {/* The composition fills the surface content area entirely. The
          chrome (LIVE + label + crosshairs) sits in the absolute slots. */}
      <div className="relative isolate flex h-full w-full items-center justify-center overflow-hidden pb-10 lg:pb-14">
        {/* Bed ribbon — full cell, lower opacity (was 0.30). */}
        <motion.img
          src={RIBBON_CUTOUT}
          alt=""
          aria-hidden="true"
        ...
        <div className="relative z-10 w-[82%] max-w-[360px] aspect-[1.586/1] transition-transform duration-500 ease-out group-hover:-translate-y-2">
          ...
          >
            ...
            {/* Cyan top edge — the brand cue (interior to the card). */}
            <span
              aria-hidden="true"
            ...
            {/* Chip — top-left. Navy stroke against the light surface.
                Carries the resting cyan pulse on the 8s beat. */}
            <span
              aria-hidden="true"
            ...
            >
              ...
              {/* Cyan halo behind the chip — gentle 8s breathing. */}
              {!reduced && (
                <motion.span
                  aria-hidden="true"
                  className="pointer-events-none absolute inset-[-6px] rounded-md"
                  style={{
                    background:
                      "radial-gradient(60% 60% at 50% 50%, rgba(34,211,238,0.45), transparent 72%)",
                  }}
