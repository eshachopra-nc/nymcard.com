...
import { AMBER, Crumb, useScrollGate, useSteps } from "./migration-kit";
...
export function LegacyCostTimeline({ className }: { className?: string }) {
  ...
  return (
    <div
      ...
      {...bind}
      className={cn(
        "relative isolate flex h-full w-full flex-col overflow-hidden rounded-xl border p-6 sm:p-8",
        // Deliberately heavier, muted bed — slate-navy, no cyan glow.
        "border-surface-border-subtle bg-surface-soft",
        "dark:border-surface-dark-border dark:bg-surface-dark-base",
        className,
      )}
    >
      {/* faint heavy wash — a muted navy haze, no cool bloom (the burden read). */}
      <span
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 z-0"
        style={{
          background: `linear-gradient(180deg, ${withAlpha(visual.navy, 0.04)}, transparent 60%)`,
        }}
      />
      ...
      <div className="relative z-10 mt-5 flex items-center justify-between border-t border-surface-border-subtle pl-[116px] pt-3 dark:border-surface-dark-border sm:pl-[132px]">
        {PHASES.map((p) => (
          <span
            key={p}
            className="font-mono text-[9.5px] uppercase tracking-[0.12em] text-text-muted dark:text-text-dark-muted"
          >
            {p}
          </span>
        ))}
