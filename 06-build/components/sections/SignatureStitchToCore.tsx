...
export function SignatureStitchToCore({
  ...
  vendors?: WebVendor[];
  /**
   * `true` (default) — the collapse sits inside its own glass card on the
   * kinetic-ribbon field (the standalone / nCore-centerpiece / social use).
   * `false` — bare: the convergence + the resolved NCoreStack render with NO
   * card chrome, so they sit directly on the HOST section's atmosphere at full
   * presence (the homepage §4 use — the stack is the morph's destination, not a
   * surface squashed in a card).
   */
  framed?: boolean;
  className?: string;
}) {
  ...
  return <CollapseToCore vendors={vendors} framed={framed} className={className} />;
}
...
function CollapseToCore({
  ...
  // The converging tangle + the resolving stack — shared between framed and
  // bare. In both, the nodes drift inward + fade on one timeline as the stack
  // scales up (crossfade, never a half-empty mid-state).
  const inner = (
    <>
  ...
  // ── Bare (framed=false) — the §4 in-page use. No card chrome: the stack is
  // the morph's destination at full presence, sitting on the HOST section's
  // atmosphere (NCoreFoundation supplies the violet field + kinetic ribbon). ──
  if (!framed) {
    return (
      <div
        role="img"
        aria-label={COLLAPSE_LABEL}
        className={cn("relative isolate flex w-full items-center justify-center", className)}
      >
        {inner}
      </div>
    );
  ...
  // ── Framed (default) — the standalone / nCore-centerpiece / social use.
  return (
    <div
      role="img"
      aria-label={COLLAPSE_LABEL}
...
const COLLAPSE_LABEL =
  "The separate vendor systems converge and resolve into the single nCore platform — six product layers (Cards, Lending, Money Movement, Settlement, Financial Crime, Reconciliation) on the nCore engine, with a cyan wave rising through every layer.";
