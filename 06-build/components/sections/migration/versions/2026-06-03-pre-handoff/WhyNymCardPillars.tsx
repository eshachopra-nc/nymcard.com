...
// agentic AI runs it. Two equal-weight pillars side by side, each a canonical
// GlassPanel floating on its own GlassAtmosphere field (design-system.md §8.1 —
// glass NEVER over a solid colour). Each pillar carries a heading, a line of
// body, and a quiet supporting-UI motif left as a labelled UIPlaceholder for
// the ui-ux-designer (a parallel-run meter for the left, an agent activity feed
// for the right). No section eyebrow (owner rule + CLAUDE.md v1.5).
//
  ...
  pillars: [
    {
      ...
      tone: "cyan" as AtmosphereTone,
      motifLabel: "Parallel-run meter — portfolio shadowing live processor",
    },
    {
      ...
      tone: "indigo" as AtmosphereTone,
      motifLabel: "Agent activity feed — mapping, testing, reconciling",
    },
  ],
} as const;
      ...
      <div className="grid gap-6 lg:grid-cols-2 lg:gap-8">
        {COPY.pillars.map((pillar, i) => {
          ...
          return (
            ...
            >
              {/* Rich cool field — the frost reads only over atmosphere (§8.1). */}
              <GlassAtmosphere tone={pillar.tone} />
              <GlassPanel className="relative z-10 flex h-full flex-col">
                <h3 className="font-display text-xl font-semibold tracking-tight text-text-primary sm:text-[1.5rem] dark:text-text-on-brand">
                  {pillar.title}
                </h3>
                <p className="mt-3 font-body text-base leading-relaxed text-text-secondary dark:text-text-dark-secondary">
                  {pillar.body}
                </p>
                {/* Quiet supporting-UI motif — a real product detail floating on
                    the glass field. */}
                <div className="mt-7 flex-1">{motif}</div>
              </GlassPanel>
            </div>
          );
        })}
