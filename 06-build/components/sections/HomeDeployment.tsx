...
export function HomeDeployment() {
  ...
  return (
    <Section bg="soft" className="py-16 sm:py-20 lg:py-24" backgrounds={<SectionAtmosphere anchor="bottom" />}>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        {MODELS.map((m, i) => (
          <motion.div
            ...
            className="nc-card-hover rounded-2xl border border-white/60 bg-surface-white/70 p-8 backdrop-blur-xl dark:border-white/10 dark:bg-surface-dark-elevated/55"
          >
            {/* Line-art sits on a faint cool field (a contained tint, restrained
                — never a wash) so the diagram reads dimensional, not stark on
                bare white. Calm, to pair with §7. */}
            <div
              className="mb-6 grid h-40 place-items-center overflow-hidden rounded-xl border border-surface-border-subtle/60 dark:border-surface-dark-border"
              style={{
                background:
                  `radial-gradient(120% 110% at 18% 0%, ${withAlpha(visual.cyan, 0.07)}, transparent 60%),` +
                  `radial-gradient(110% 120% at 90% 110%, ${withAlpha(visual.indigo, 0.06)}, transparent 62%)`,
              }}
            >
              {ART[m.key]}
            </div>
